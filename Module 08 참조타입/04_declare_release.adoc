...
위의 코드는 account라는 BankAccount 타입 변수를 선언합니다. 값 타입 변수일 경우 변수를 선언하고 초기화하지 않으면 기본 값(int의 경우 0)이 기본적으로 할당됩니다. 참조 타입인 경우에는 참조 없음(null)이 할당됩니다.

참조 타입 변수를 초기화하려면 new 연산자를 사용해야 합니다. new 연산자는 새 객체를 생성하고 객체가 존재하는 곳의 참조를 return합니다.
