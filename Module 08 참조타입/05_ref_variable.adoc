...
컴파일러는 프로그램이 실행되기 전에 잘못된 참조를 검출할 수 있습니다. 아래 예에서, BankAccount 타입 참조 변수 account는 선언되었지만 초기화 되지 않았습니다. 컴파일러는 이 오류를 검출하고 컴파일하지 않습니다.
...
참조 타입 변수의 값이 null일 때 변수를 사용하면 런타임은 NullPointerException을 던집니다. 이 예외는 try-catch 문을 사용해서 잡을 수 있습니다.
...
link:./06_comparisiopn.adoc[다음: 값 타입의 비교와 참조 타입의 비교]